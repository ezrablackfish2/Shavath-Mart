...
function AboutSelector ( {setSelectedService, selectedService, selectedDrop, setSelectedDrop} : Props) {
  ...
  return (
    <div className={isOpen ? styles.dropdownopen2 : styles.dropdown2} onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
      <div className={(selectedDrop === "About" || selectedDrop === "all") ? ( isOpen ? styles.select2open : styles.select2 ) : (styles.hiddenword)} onClick={toggleDropdown}>
        <div className={styles.selectorword}>About</div> <div className={styles.imagebox}><img src="/dropdown.png" className={styles.dropdownarrow} /></div>
      </div>
      <div className={styles.options2}>
		{services.map((option, index) => (
                                <div className={styles.option2} onClick={() => setSelectedService(option)}>
                                {option}
                                </div>
                        ))}
      </div>
